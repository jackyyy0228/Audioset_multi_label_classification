...
from .DLMA_attention import DLMAAttention, DLMAAddAttention
...
class DecoderRNN(BaseRNN):
    ...
    def __init__(self, vocab_size, max_len, hidden_size, sos_id, eos_id,
        ...
        if use_attention:
            self.attention = DLMAAddAttention(self.hidden_size, self.output_size)
            self.decoder = SimpleDecoder(self.output_size*2, self.output_size, 
                                         sampling_type = sampling_type)
            '''
            self.attention = DLMAAttention(self.hidden_size)
